// Cloudflare Pages Function: proxies wallet lookups to Helius so the API key
// never reaches the browser. Key lives in a Cloudflare Pages environment
// variable, HELIUS_API_KEY, set in your project's Settings -> Environment
// variables (not in this file).
//
// This file at functions/helius-proxy.js is automatically routed by Cloudflare
// Pages to the path /helius-proxy — no extra config needed.
//
// Includes the same in-memory cache and per-IP rate limiter as the Netlify
// version. They live in module scope, so they persist for the life of a warm
// Worker isolate but reset when Cloudflare spins up a new one, and aren't
// shared across every edge location handling your traffic. Good enough to
// absorb repeat lookups and casual hammering; for a hard guarantee at real
// scale, Cloudflare's Cache API or a KV namespace would be the next step up.
...
// Pages Functions convention: onRequest handles every HTTP method for this route.
export async function onRequest(context){
  ...
  if(!HELIUS_API_KEY){
    return json({ error: 'HELIUS_API_KEY is not set in Cloudflare Pages environment variables.' }, 500);
  }

  // Cloudflare provides the real client IP via this header — no need to parse
  // x-forwarded-for like on Netlify.
